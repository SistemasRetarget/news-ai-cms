---
last_updated: 2026-05-07T18:00:00Z
account: empresa (sistemas@retarget.cl)
---

# 🤝 HANDOFF: HISTÓRICO DE SESIÓN

⚠️ **IMPORTANTE CRÍTICO:**
- **SHEETS ES LA FUENTE DE VERDAD:** https://docs.google.com/spreadsheets/d/1murmG-pdc5GkJ1CYc4_1UISRTcipMxPYv2jiH_-7ZIY/edit#gid=854866235
- **LEE SHEETS PRIMERO AL INICIAR** — actualizado en tiempo real
- HANDOFF es solo referencia histórica

---

## 📊 GANTT — Estado de Tickets (desde Sheets)

| Ticket | Negocio | Sitio | Problema | Solución | Estado | Fecha Solicitud | Fecha Cierre |
|--------|---------|-------|----------|----------|--------|-----------------|-------------|
| F-01 | Futangue EN | parquefutangue.com/en | Botones IBE apuntaban a paso1.cgi | Links corregidos a /en/programs/ | ✅ Completado | 04-may | 05-may |
| F-02 | Futangue EN | parquefutangue.com/en | Sección Fly Fishing sin imagen/texto | Sección agregada con programa | ✅ Completado | 04-may | 05-may |
| F-03 | Futangue EN | parquefutangue.com/en | See More redirigían al home ES | Links corregidos a /en/hotel/ y /en/private-reserve/ | ✅ Completado | 04-may | 05-may |
| F-04 | Futangue EN | parquefutangue.com/en | When to Visit: tipografía + Climate Zones | Bloque eliminado, tipografía alineada | ✅ Completado | 04-may | 05-may |
| F-05 | Futangue EN | parquefutangue.com/en | Logo Michelin faltaba en Hotel EN | Logo ya estaba (verificado) | ✅ Completado | 04-may | 05-may |
| F-06a | Futangue EN | parquefutangue.com/en | Contact Us → /en/contact-us-2/ (inexistente) | URL corregida a /en/contact-us/ | ✅ Completado | 04-may | 05-may |
| F-06b | Futangue EN | parquefutangue.com/en | Footer EN: Cabins → # (link roto) | Link actualizado a cabanas.parquefutangue.com | ✅ Completado | 04-may | 05-may |
| F-06c | Futangue EN | parquefutangue.com/en | Footer EN: Help → /en/contact-us-2/ | URL corregida a /en/contact-us/ | ✅ Completado | 04-may | 05-may |
| F-06h | Futangue EN | parquefutangue.com/en | Fly Fishing sin botón descarga PDF | PDF subido (ID 6534), botón agregado | ⏳ Pendiente | 07-may | — |
| T-01 | TAC | termasaguascalientes.cl | Landing excursiones desactualizada | Página clonada (ID 14807 → 20645), tabs reemplazados | ⏳ Pendiente | 29-apr | — |
| T-02 | Puyehue | puyehue.cl | Botones Ven por el Día centrados + borde negro | 7 botones: align=left, border=none | ⏳ Pendiente | 27-apr | — |
| T-03 | Cabañas | cabanas.parquefutangue.com | Subdominio sin WordPress | WordPress instalado + 5 páginas clonadas | ✅ Completado | 29-apr | 05-may |
| T-04 | Pueblo La Dehesa | Shopify Algodones | Apps no usadas generan costos | Requiere 2FA de Leignayih | 🔴 Bloqueado | 29-apr | — |
| T-05 | Futangue EN | parquefutangue.com/en | Duplicado de sección en home | Eliminada | ✅ Completado | 05-may | 05-may |
| T-06 | Puyehue | puyehue.cl | Botones Daypass/Destino estilos inconsistentes | 6 botones actualizados: borde + hover #2d9c6c | ✅ Completado | 07-may | 07-may |
| T-07 | Puyehue | puyehue.cl | REQ-003: Botones Daypass — aclaraciones | — | ✅ Completado | 05-may | — |
| T-08 | Puyehue | puyehue.cl | REQ-003: Botones Daypass — aclaraciones | — | ✅ Completado | 05-may | — |
| T-09 | TAC | termasaguascalientes.cl | Fwd: Ajustes HTP ven por el día | — | ⏳ Pendiente | 28-apr | — |
| T-10 | Puyehue | puyehue.cl | Fwd: Ajustes landing excursiones TAC | — | ⏳ Pendiente | 29-apr | — |
| T-11 | Futangue EN | parquefutangue.com/en | REQ-004: Landing Nueva — 5 aclaraciones | — | ⏳ Pendiente | 05-may | — |
| T-12 | TAC | termasaguascalientes.cl | REQ-001: Botones — tamaño exacto | — | ⏳ Pendiente | 05-may | — |
| T-13 | Futangue EN | parquefutangue.com/en | REQ-005: Futangue | — | ⏳ Pendiente | 05-may | — |

---

## 🗺️ PROYECTOS ACTIVOS

| Negocio | Sitio | Stack | Estado |
|---------|-------|-------|--------|
| **Futangue** | parquefutangue.com | WordPress + WPBakery + WPML | En desarrollo (F-01 a F-06h) |
| **TAC** | termasaguascalientes.cl | Elementor | En desarrollo (T-01) |
| **Cabañas** | cabanas.parquefutangue.com | WordPress + Elementor | Completado (T-03) |
| **Pueblo La Dehesa** | puebloladehesa.cl | Next.js 15 + Payload CMS | En desarrollo (T-04 bloqueado) |
| **CRM Comunitario** | cmr.socialtarget.cl | Next.js | Activo en revisión Meta |

**NOTA:** Puyehue completado (T-02, T-06) — removido del roadmap principal.

---

## 🎯 PRÓXIMOS PASOS

1. **Verificar Sheets** — antes de cualquier sesión, leer el tracking master
2. **TAC (T-01)** — validar landing excursiones clonada
3. **Futangue (F-06h)** — validar PDF Fly Fishing
4. **Pueblo La Dehesa (T-04)** — esperar 2FA de Leignayih
5. **REQs pendientes aclaración** — T-11, T-12, T-13 (pedir detalles a Leignayih)

---

*Generado: 2026-05-07*  
*Sistema de Memoria Sincronizada en producción*
